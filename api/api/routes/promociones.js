...
router.post('/', async (req, res) => {
  try {
    ...
    let insertQuery = `INSERT INTO promociones (sorteo_id, cantidad_tickets, precio_total, descuento, activa) VALUES (?, ?, ?, '0.00', true)`;
    if (DB_TYPE === 'postgres') insertQuery += ' RETURNING id';

    const [result] = await pool.execute(insertQuery, [sorteo_id, cantidad, precioTotal]);
    const insertId = DB_TYPE === 'postgres'
      ? (Array.isArray(result) && result[0]?.id ? result[0].id : null)
      : result.insertId;

    if (!insertId) {
      return res.status(201).json({ sorteo_id, cantidad_tickets: cantidad, precio_total: precioTotal });
    }
    const [newRow] = await pool.execute('SELECT * FROM promociones WHERE id = ?', [insertId]);
    res.status(201).json(newRow[0] || { id: insertId, sorteo_id, cantidad_tickets: cantidad, precio_total: precioTotal });
  } catch (error) {
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (activa !== undefined) { updates.push('activa = ?'); values.push(Boolean(activa)); }
...
router.delete('/:id', async (req, res) => {
  try {
    await pool.execute('DELETE FROM promociones WHERE id = ?', [req.params.id]);
