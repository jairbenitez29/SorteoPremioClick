...
// Ejecutar pago PayPal desde WEB (sin token de usuario, usa paymentId para identificar el pago)
router.post('/paypal/execute-web', async (req, res) => {
  try {
    const { paymentId, payerId } = req.body;

    if (!paymentId || !payerId) {
      return res.status(400).json({ error: 'Faltan parámetros: paymentId y payerId son obligatorios' });
    }

    // Buscar el pago pendiente por paymentId en datos_pago
    const { DB_TYPE } = require('../config/database');
    let searchQuery;
    if (DB_TYPE === 'postgres') {
      searchQuery = `SELECT * FROM pagos WHERE datos_pago::json->>'paymentId' = $1 AND estado = 'pendiente' ORDER BY created_at DESC LIMIT 1`;
    } else {
      searchQuery = `SELECT * FROM pagos WHERE JSON_EXTRACT(datos_pago, '$.paymentId') = ? AND estado = 'pendiente' ORDER BY created_at DESC LIMIT 1`;
    }
    ...
    paypal.payment.execute(paymentId, execute_payment_json, async (error, payment) => {
      if (error) {
        console.error('Error al ejecutar pago PayPal (web):', error);
        await pool.execute('UPDATE pagos SET estado = "fallido" WHERE id = ?', [pagoIdFinal]);
      ...
      if (payment.state === 'approved') {
        ...
        res.json({ success: true, message: 'Pago completado correctamente', ticketIds });
      } else {
        await pool.execute('UPDATE pagos SET estado = "fallido" WHERE id = ?', [pagoIdFinal]);
        res.status(400).json({ error: 'Pago no aprobado por PayPal' });
      }
    });
  } catch (error) {
    console.error('Error al ejecutar pago PayPal (web):', error);
